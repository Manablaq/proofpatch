...
class ProofPatchReviewCommitLogic:
	def __init__(self):self.policies={};self.proposals={};self.releases={};self.incidents={};self.active_proposal_by_target={};self.actions=[]
	def _now(self):return self.now
	...
	def _release_active(self,target,proposal_id):
		if self.active_proposal_by_target.get(target,u256(0))==proposal_id:self.active_proposal_by_target[target]=u256(0)
	def _proposal_release_id(self,proposal):return'release-'+str(proposal.proposal_id)+'-'+proposal.candidate_code_hash[:16]
	def proposal(self,proposal_id,result):
		proposal=self._require_proposal(proposal_id);policy=self.policies[proposal.target];proposal.reviewed_at=u64(self._now());proposal.last_review_code=str(result.get('error_code',''))
		if result['kind']==REVIEW_REPAIR:proposal.status=STATUS_REPAIR;return
		if result['kind']==REVIEW_RETRY:proposal.status=STATUS_RETRY;return
		if result['kind']!=REVIEW_DECISION:raise gl.vm.UserError('Unexpected review result')
		if result['decision']==DECISION_REJECT:proposal.status=STATUS_REJECTED;self._release_active(proposal.target,proposal_id);return
		if result['decision']!=DECISION_APPROVE:raise gl.vm.UserError('Unexpected decision')
		proposal.status=STATUS_QUEUED;proposal.execution_deadline=u64(self._now()+int(policy.execution_timeout_seconds));self.actions.append({'kind':'upgrade','args':[str(proposal.target),int(proposal_id),proposal.candidate_code_hash]})
	def assurance(self,proposal_id,result,primary_url,primary_id,corroboration_url,corroboration_id):
		proposal=self._require_proposal(proposal_id);release_id=self._proposal_release_id(proposal)
		if result['kind']==REVIEW_REPAIR:proposal.status=STATUS_ASSURANCE_REPAIR;proposal.last_review_code=str(result.get('error_code',''));return
		if result['kind']==REVIEW_RETRY:proposal.status=STATUS_ASSURANCE_RETRY;proposal.last_review_code=str(result.get('error_code',''));return
		proposal.last_review_code=str(result.get('error_code',''))
		if result.get('decision')==DECISION_APPROVE:proposal.status=STATUS_CERTIFICATION_QUEUED;self.releases[release_id].status=STATUS_CERTIFICATION_QUEUED;self.actions.append({'kind':'activate','args':[str(proposal.target),release_id,proposal.candidate_code_hash]});return
		proposal.status=STATUS_INCIDENT_OPEN;now=self._now();policy=self.policies[proposal.target];self.incidents['assurance-'+release_id]=IncidentRecord(incident_id='assurance-'+release_id,target=proposal.target,release_id=release_id,installed_code_hash=proposal.candidate_code_hash,incident_type='ASSURANCE_FAILURE',primary_url=primary_url,primary_evidence_id=primary_id,corroboration_url=corroboration_url,corroboration_evidence_id=corroboration_id,policy_fingerprint=proposal.policy_fingerprint,assurance_manifest_hash=proposal.assurance_manifest_hash,recovery_capsule_hash=proposal.recovery_capsule_hash,opened_at=u64(now),expires_at=u64(now+int(policy.proposal_ttl_seconds)),reviewed_at=u64(now),recovery_deadline=u64(0),status=STATUS_INCIDENT_OPEN,last_review_code='ASSURANCE_FAILED',recovery_authorized=False);self.releases[release_id].status=STATUS_INCIDENT_OPEN
	def incident(self,incident_id,result):
		incident=self.incidents[incident_id];release=self.releases[incident.release_id];proposal=self.proposals[release.proposal_id];incident.reviewed_at=u64(self._now());incident.last_review_code=str(result.get('error_code',''))
		if result['kind']==REVIEW_REPAIR:incident.status=STATUS_INCIDENT_REPAIR;return
		if result['kind']==REVIEW_RETRY:incident.status=STATUS_INCIDENT_RETRY;return
		if result.get('decision')!=DECISION_APPROVE:incident.status=STATUS_INCIDENT_DISMISSED;release.status=STATUS_INSTALLED_PROVISIONAL if self.target_mode==MODE_PROVISIONAL else STATUS_CERTIFIED;proposal.status=STATUS_INSTALLED_PROVISIONAL if self.target_mode==MODE_PROVISIONAL else STATUS_CERTIFIED;return
		incident.status=STATUS_INCIDENT_CONFIRMED;incident.recovery_authorized=True;incident.recovery_deadline=u64(self._now()+int(self.policies[incident.target].execution_timeout_seconds));release.status=STATUS_INCIDENT_CONFIRMED;self.actions.append({'kind':'recover','args':[incident_id,incident.release_id,incident.recovery_capsule_hash]})
...
class ProofPatchReviewCommitEngine(gl.Contract):
	...
	def _record(self,cls,raw):
		...
		if cls is TargetPolicy or cls is UpgradeProposal or cls is ReleaseRecord or cls is IncidentRecord:value[1]=Address(value[1])
	...
	def _load(self,logic,data):
		...
		for(key,cls,store)in(('policy',TargetPolicy,logic.policies),('proposal',UpgradeProposal,logic.proposals),('release',ReleaseRecord,logic.releases),('incident',IncidentRecord,logic.incidents)):
			if data.get(key)is not None:value=self._record(cls,data[key]);store[value.target if key=='policy'else value.proposal_id if key=='proposal'else value.release_id if key=='release'else value.incident_id]=value
		if data.get('active_target')is not None:logic.active_proposal_by_target[Address(data['active_target'])]=u256(data.get('active_value',0))
		logic.target_mode=str(data.get('target_mode',''))
	def _result(self,raw,expected):
		try:result=json.loads(raw)
		except Exception:raise gl.vm.UserError('Review engine returned invalid JSON')
		if not isinstance(result,dict):raise gl.vm.UserError('Review engine returned an invalid result')
		for(key,value)in expected.items():
			if result.get(key)!=value:raise gl.vm.UserError('Review engine result binding mismatch')
		return result
	def _state(self,view,kind,key):return json.loads(view.get_state_record(kind,key))
	def _request_state(self,operation,data,view):
		args=data.get('args',[])
		if operation=='proposal':
			proposal=self._state(view,'proposal',str(int(args[0])));policy=self._state(view,'policy',proposal['target']);result=self._result(ProofPatchReviewEngine(Address(REVIEW_ENGINE)).view(state=StorageType.LATEST_FINAL).get_proposal_result(u256(args[0])),{'target':proposal['target'],'proposal_id':int(args[0]),'parent_code_hash':proposal['parent_code_hash'],'candidate_code_hash':proposal['candidate_code_hash'],'policy_fingerprint':proposal['policy_fingerprint'],'evidence_set_hash':proposal['evidence_set_hash'],'assurance_manifest_hash':proposal['assurance_manifest_hash'],'recovery_capsule_hash':proposal['recovery_capsule_hash']})
			if proposal['status']!=STATUS_REVIEW_PENDING:raise gl.vm.UserError('Proposal is not awaiting review callback')
		...
		if operation=='incident':
			incident=self._state(view,'incident',args[0]);release=self._state(view,'release',incident['release_id']);proposal=self._state(view,'proposal',str(release['proposal_id']));policy=self._state(view,'policy',incident['target']);result=self._result(ProofPatchAssuranceIncidentEngine(Address(ASSURANCE_REVIEW_ENGINE)).view(state=StorageType.LATEST_FINAL).get_incident_result(args[0]),{'incident_id':args[0],'target':incident['target'],'release_id':incident['release_id'],'installed_code_hash':incident['installed_code_hash'],'policy_fingerprint':incident['policy_fingerprint'],'recovery_capsule_hash':incident['recovery_capsule_hash']})
			if incident['status']!=STATUS_INCIDENT_REVIEW_PENDING:raise gl.vm.UserError('Incident is not awaiting review callback')
			target_mode=ProofPatchTarget(Address(incident['target'])).view(state=StorageType.LATEST_FINAL).proofpatch_release_mode();return{'incident':incident,'release':release,'proposal':proposal,'policy':policy,'target_mode':target_mode,'result':result}
	...
	def _patch(self,logic,operation):
		...
		for value in logic.policies.values():out['policies'].append(self._encode(value))
		for value in logic.proposals.values():out['proposals'].append(self._encode(value))
		for value in logic.releases.values():out['releases'].append(self._encode(value))
		for value in logic.incidents.values():out['incidents'].append(self._encode(value))
		for(key,value)in logic.active_proposal_by_target.items():out['active'].append([str(key),int(value)])
		return out
	@gl.public.write
	def execute(self,operation:str,request:str)->None:
		if gl.message.sender_address!=self.governor:raise gl.vm.UserError('Only the bound governor may execute review commits')
		data=json.loads(request);state=self._request_state(operation,data,ProofPatchGovernorV2(self.governor).view(state=StorageType.LATEST_FINAL))
		for(key,value)in state.items():
			if key!='result':data[key]=value
		logic=ProofPatchReviewCommitLogic();self._load(logic,data);args=data.get('args',[])
		if operation=='proposal':logic.proposal(args[0],state['result'])
		elif operation=='assurance':logic.assurance(args[0],state['result'],args[1],args[2],args[3],args[4])
		elif operation=='incident':logic.incident(args[0],state['result'])
		else:raise gl.vm.UserError('Unknown review operation')
		payload=json.dumps(self._patch(logic,operation),separators=(',',':'));ProofPatchGovernorV2(self.governor).emit(on='finalized').apply_review_result(operation,payload)
