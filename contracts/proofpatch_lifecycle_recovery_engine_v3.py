...
class ProofPatchRecoveryLifecycleLogic:
    ...
    def expire_provisional(self, release_id: str) -> None:
        ...
        if incident_id not in self.incidents:
            ...
            proposal.status = STATUS_INCIDENT_CONFIRMED
            self.actions.append({'kind': 'recover', 'args': [incident_id, release_id, release.recovery_capsule_hash]})
...
class ProofPatchRecoveryLifecycleEngine(gl.Contract):
    ...
    def _load(self, logic: ProofPatchRecoveryLifecycleLogic, data: dict[object, object]) -> None:
        ...
        if records.get('incident') is not None:
            i = self._record(IncidentRecord, records['incident']); logic.incidents[i.incident_id] = i
    ...
    def execute(self, operation: str, request: str) -> None:
        ...
        data = json.loads(request); logic = ProofPatchRecoveryLifecycleLogic(); self._load(logic, data); args = data.get('args', [])
