...
class ProofPatchTimeoutLifecycleLogic:
...
class ProofPatchTimeoutLifecycleEngine(gl.Contract):
	...
	def _patch(self,logic,operation):
		...
		for(k,v)in logic.active_proposal_by_target.items():out[D].append([str(k),int(v)])
		return out
	@gl.public.write
	def execute(self,operation,request):
		...
		if operation!='timeout':raise gl.vm.UserError('Unsupported timeout lifecycle operation')
		data=json.loads(request);logic=ProofPatchTimeoutLifecycleLogic();self._load(logic,data);logic.mark_timeout(*data.get('args',[]));ProofPatchGovernorV2(self.governor).emit(on='finalized').apply_lifecycle_result(operation,json.dumps(self._patch(logic,operation),separators=(',',':')))
