...
class ProofPatchSummaryEngine(gl.Contract):
    ...
    def get_candidate_code(self, proposal_id: u256) -> bytes:
        ...
        if value['status'] != 'UPGRADE_QUEUED':
            raise gl.vm.UserError('Candidate is not authorized for installation')
        if self._now() > int(value['execution_deadline']):
    ...
    def get_recovery_release_id(self, incident_id: str) -> str:
        ...
        if release is None:
            return ''
        proposal = self._get('proposal', str(release['proposal_id']))
        return '' if proposal is None else proposal['recovery_release_id']
    ...
    def get_recovery_code(self, incident_id: str) -> bytes:
        ...
        if incident['status'] not in ('INCIDENT_CONFIRMED', 'RECOVERY_QUEUED', 'RECOVERY_RETRY_REQUIRED'):
            raise gl.vm.UserError('Recovery is not authorized')
        release = self._get('release', incident['release_id'])
        proposal = self._get('proposal', str(release['proposal_id']))
        return bytes.fromhex(proposal['recovery_code'])
    ...
    def is_upgrade_authorized(self, proposal_id: u256, target: str, candidate_hash: str) -> bool:
        ...
        if self._missing(p) or p['status'] != 'UPGRADE_QUEUED' or p['target'].lower() != str(Address(target)).lower() or p['candidate_code_hash'] != candidate_hash.lower() or self._now() > int(p['execution_deadline']):
            return False
    ...
    def is_activation_authorized(self, proposal_id: u256, release_id: str, candidate_hash: str) -> bool:
        p = self._get('proposal', str(int(proposal_id)))
        r = self._get('release', release_id)
        return not self._missing(p) and not self._missing(r) and p['status'] == 'CERTIFICATION_QUEUED' and release_id == 'release-' + str(p['proposal_id']) + '-' + p['candidate_code_hash'][:16] and candidate_hash.lower() == p['candidate_code_hash'] and r['status'] == 'CERTIFICATION_QUEUED'

    @gl.public.view
    def is_registration_authorized(self, target: str, release_id: str, code_hash: str) -> bool:
        p = self._get('policy', target)
        r = self._get('release', release_id)
        return not self._missing(p) and not self._missing(r) and p['current_release_id'] == release_id and p['current_code_hash'] == code_hash.lower() and r['status'] == 'REGISTERED_PARENT'

    @gl.public.view
    def is_recovery_authorized(self, incident_id: str, release_id: str, recovery_hash: str) -> bool:
        i = self._get('incident', incident_id)
        r = self._get('release', release_id)
    ...
    def get_proposal_count(self) -> u256:
        value = self._get('counts', '')
        return u256(0 if self._missing(value) else value['proposal_count'])

    @gl.public.view
    def get_proposal_status(self, proposal_id: u256) -> str:
        value = self._get('proposal', str(int(proposal_id)))
        return 'UNKNOWN' if self._missing(value) else value['status']

    @gl.public.view
    def get_candidate_hash(self, proposal_id: u256) -> str:
        value = self._get('proposal', str(int(proposal_id)))
        return '' if self._missing(value) else value['candidate_code_hash']

    @gl.public.view
    def get_proposal_release_id(self, proposal_id: u256) -> str:
        value = self._get('proposal', str(int(proposal_id)))
        return '' if self._missing(value) else 'release-' + str(value['proposal_id']) + '-' + value['candidate_code_hash'][:16]

    @gl.public.view
    def get_evidence_set_hash(self, proposal_id: u256) -> str:
        value = self._get('proposal', str(int(proposal_id)))
        return '' if self._missing(value) else value['evidence_set_hash']

    @gl.public.view
    def get_policy_fingerprint(self, target: str) -> str:
        value = self._get('policy', target)
        return '' if self._missing(value) else value['policy_fingerprint']

    @gl.public.view
    def get_policy_kernel_hash(self, target: str) -> str:
        value = self._get('policy', target)
        return '' if self._missing(value) else value['proofpatch_kernel_hash']

    @gl.public.view
    def get_current_code_hash(self, target: str) -> str:
        value = self._get('policy', target)
        return '' if self._missing(value) else value['current_code_hash']

    @gl.public.view
    def get_current_version(self, target: str) -> str:
        value = self._get('policy', target)
        return '' if self._missing(value) else value['current_version']

    @gl.public.view
    def get_current_release_id(self, target: str) -> str:
        value = self._get('policy', target)
        return '' if self._missing(value) else value['current_release_id']

    @gl.public.view
    def get_active_proposal(self, target: str) -> u256:
        value = self._get('active', target)
        return u256(0 if self._missing(value) else value['value'])

    @gl.public.view
    def read(self, kind: str, key: str) -> str:
        if kind.endswith('_summary'):
            return getattr(self, 'get_' + kind)(key if kind != 'proposal_summary' else u256(int(key)))
        if kind in ('candidate_code', 'recovery_code'):
            return (self.get_candidate_code(u256(int(key))) if kind == 'candidate_code' else self.get_recovery_code(key)).hex()
        if kind == 'recovery_release_id':
            return self.get_recovery_release_id(key)
        if kind.startswith('is_'):
            args = json.loads(key)
            result = getattr(self, kind)(*args)
            return '1' if result else '0'
        if kind == 'proposal_count':
            return str(int(self.get_proposal_count()))
        if kind == 'active_proposal':
            return str(int(self.get_active_proposal(key)))
