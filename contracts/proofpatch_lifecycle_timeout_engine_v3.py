...
class ProofPatchTimeoutLifecycleEngine(gl.Contract):
    ...
    def _load(self,logic,data):
        ...
        records=data.get('records',{});logic.proposal_count=u256(data.get('proposal_count',0));logic.release_count=u256(data.get('release_count',0));logic.installed_candidate_hashes=data.get('installed_candidate_hashes',{})
        ...
        nonfinal=data.get('target_nonfinal',{});logic.target_nonfinal_proposal_id=u256(nonfinal.get('proposal_id',0));logic.target_nonfinal_candidate_hash=str(nonfinal.get('candidate_hash',''))
    ...
    def _patch(self,logic,operation):
        ...
        for k,v in logic.active_proposal_by_target.items(): out['active'].append([str(k),int(v)])
        return out
    ...
    def execute(self,operation,request):
        ...
        data=json.loads(request);logic=ProofPatchTimeoutLifecycleLogic();self._load(logic,data);logic.mark_timeout(*data.get('args',[]))
