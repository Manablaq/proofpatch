...
class ProofPatchAssuranceLogic:
    ...
    def _raw_github_owner(self, prefix: str) -> str:
        ...
        if len(parts) != 3 or parts[2] != '':
            return ''
        if not self._is_canonical_raw_segment(parts[0]) or not self._is_canonical_raw_segment(parts[1]):
            return ''
        return parts[0]

    def _is_immutable_url(self, url: str, prefix: str) -> bool:
        if len(url.encode('utf-8')) > MAX_URL_BYTES or self._raw_github_owner(prefix) == '':
            return False
        if not url.startswith(prefix):
            return False
        parts = url[len(prefix):].split('/', 1)
        if len(parts) != 2 or len(parts[0]) != 40 or any(char not in '0123456789abcdef' for char in parts[0]):
            return False
    ...
    def _assure(self, view: typing.Any, args: list[typing.Any], now: int) -> dict[object, object]:
        ...
        release_id = 'release-' + str(proposal['proposal_id']) + '-' + proposal['candidate_code_hash'][:16]
        release = json.loads(view.get_state_record('release', release_id))
        ...
        if not self._is_immutable_url(args[1], policy['assurance_prefix']) or not self._is_immutable_url(args[3], policy['assurance_corroboration_prefix']):
        ...
        patch = {'operation': 'assure', 'proposals': [proposal], 'releases': [], 'used_evidence_ids': {}}
        self._reserve(view, patch, proposal['target'], policy['assurance_authority'], 'assurance_primary', args[2])
        self._reserve(view, patch, proposal['target'], policy['assurance_corroboration_authority'], 'assurance_corroboration', args[4])
        proposal['status'] = STATUS_ASSURANCE_PENDING
        proposal['reviewed_at'] = now
        proposal['last_review_code'] = 'ENGINE_PENDING'
        patch['proposals'] = [proposal]
        patch['review'] = {'proposal_id': proposal_id, 'primary_url': args[1], 'primary_evidence_id': args[2], 'corroboration_url': args[3], 'corroboration_evidence_id': args[4]}
        return patch
...
class ProofPatchAssuranceEngine(gl.Contract):
    ...
    def execute(self, operation: str, request: str) -> None:
        if gl.message.sender_address != self.governor:
            raise gl.vm.UserError('Only the bound governor may execute assurance logic')
        if operation != 'assure':
            raise gl.vm.UserError('Unknown assurance operation')
        data = json.loads(request)
        view = ProofPatchGovernorV2(self.governor).view(state=StorageType.LATEST_FINAL)
        patch = ProofPatchAssuranceLogic()._assure(view, data['args'], int(data['now']))
        ProofPatchGovernorV2(self.governor).emit(on='finalized').apply_policy_result(operation, json.dumps(patch, separators=(',', ':')))
