...
class RepairPolicyLogic:
	def _hash_text_parts(self,parts:list[str])->str:return hashlib.sha256('\x1f'.join(parts).encode(_E)).hexdigest()
	def _check_text(self,value:str,label:str,minimum:int,maximum:int)->_B:
		encoded_len=len(value.encode(_E))
	...
	def _is_immutable_url(self,url:str,prefix:str)->bool:
		if len(url.encode(_E))>MAX_URL_BYTES:return _A
	...
	def _repair_evidence(self,proposal_id:u256,candidate_source_url:str,ci_evidence_url:str,ci_evidence_id:str,audit_evidence_url:str,audit_evidence_id:str)->_B:
		proposal=self._require_proposal(proposal_id);policy=self._require_policy_owner(proposal.target)
		if proposal.status!=STATUS_REPAIR:raise gl.vm.UserError('Evidence can only be replaced from EVIDENCE_REPAIR_REQUIRED')
		if self.now>int(proposal.expires_at):raise gl.vm.UserError('Proposal has expired')
		if not self._is_immutable_url(candidate_source_url,policy.source_prefix):raise gl.vm.UserError('Replacement candidate source URL is not approved and immutable')
		if not self._is_immutable_url(ci_evidence_url,policy.ci_prefix):raise gl.vm.UserError('Replacement CI evidence URL is not approved and immutable')
		if not self._is_immutable_url(audit_evidence_url,policy.audit_prefix):raise gl.vm.UserError('Replacement audit evidence URL is not approved and immutable')
		if ci_evidence_id==audit_evidence_id:raise gl.vm.UserError('CI and audit evidence identifiers must be distinct')
		self._reserve_evidence_id(proposal.target,policy.ci_authority,'ci',ci_evidence_id);self._reserve_evidence_id(proposal.target,policy.audit_authority,'audit',audit_evidence_id);proposal.candidate_source_url=candidate_source_url;proposal.ci_evidence_url=ci_evidence_url;proposal.ci_evidence_id=ci_evidence_id;proposal.audit_evidence_url=audit_evidence_url;proposal.audit_evidence_id=audit_evidence_id;proposal.evidence_set_hash=self._evidence_set_hash(candidate_source_url,ci_evidence_url,ci_evidence_id,audit_evidence_url,audit_evidence_id,proposal.assurance_manifest_hash,proposal.recovery_capsule_hash);proposal.status=STATUS_PROPOSED;proposal.last_review_code=''
class ProofPatchRepairEngine(gl.Contract):
	...
	def _load(self,logic:RepairPolicyLogic,data:dict[object,object])->_B:
		B='policy';A='proposal';operation=str(data.get(_F,''))
		if operation!=_G:raise gl.vm.UserError('Unknown repair policy operation')
		args=data['args'];view=ProofPatchGovernor(self.governor).view(state=StorageType.LATEST_FINAL)
		...
		def flag(kind:str,key:str,destination:str)->_B:
			if get(kind,key).get('value',_A):data[destination][key]=_C
		def evidence_key(target:str,issuer:str,kind:str,evidence_id:str)->str:return hashlib.sha256('\\x1f'.join([target,issuer,kind,evidence_id]).encode()).hexdigest()
		data[_D]={};data[A]=get(A,str(int(args[0])));target=data[A]['target'];data[B]=get(B,target);policy=data[B]
		for(issuer,kind,evidence_id)in((policy['ci_authority'],'ci',args[3]),(policy['audit_authority'],'audit',args[5])):flag('evidence',evidence_key(target,issuer,kind,evidence_id),_D)
		logic.policies={};logic.proposals={};logic.used_evidence_ids=typing.cast(dict[object,bool],data[_D]);logic.proposal_count=u256(get('counts','').get(_H,0));policy_record=self._record(TargetPolicy,data[B]);proposal_record=self._record(UpgradeProposal,data[A]);logic.policies[policy_record.target]=policy_record;logic.proposals[proposal_record.proposal_id]=proposal_record;logic.actor=Address(data['actor']);logic.now=int(data['now'])
	...
	def _record(self,cls:typing.Any,raw:dict[object,object])->typing.Any:
		...
		if cls in(TargetPolicy,UpgradeProposal):value[1]=Address(value[1])
	...
	def _patch(self,logic:RepairPolicyLogic)->dict[object,object]:
		B='proposals';A='policies';result={_F:_G,A:[],B:[],'releases':[],'incidents':[],'active':[],_D:logic.used_evidence_ids,'installed_candidate_hashes':{},'used_incident_ids':{},_H:int(logic.proposal_count)}
		for value in logic.policies.values():result[A].append(self._encode(value))
		for value in logic.proposals.values():result[B].append(self._encode(value))
		return result
	@gl.public.write
	def execute(self,operation:str,request:str)->_B:
		if gl.message.sender_address!=self.governor:raise gl.vm.UserError('Only the bound governor may execute repair policy logic')
		data=json.loads(request);data[_F]=operation;logic=RepairPolicyLogic();self._load(logic,data);logic._repair_evidence(*data['args']);ProofPatchGovernor(self.governor).emit(on='finalized').apply_policy_result(operation,json.dumps(self._patch(logic),separators=(',',':')))
