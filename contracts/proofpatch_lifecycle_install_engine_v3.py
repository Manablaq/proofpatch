...
class ProofPatchGovernorV2:
    ...
    class Write:
        ...
        def apply_lifecycle_result(self, operation: str, payload: str) -> None:
            ...
class ProofPatchInstallLifecycleLogic:
...
class ProofPatchInstallLifecycleEngine(gl.Contract):
    admin: Address
    governor: Address
    executor: Address
    ...
    def _load(self, logic: ProofPatchInstallLifecycleLogic, data: dict[object, object]) -> None:
        logic.actor=Address(data['actor']);logic.now=int(data['now'])
        records=data.get('records',{})
        logic.proposal_count=u256(data.get('proposal_count',0));logic.release_count=u256(data.get('release_count',0))
        logic.installed_candidate_hashes=data.get('installed_candidate_hashes',{})
        ...
        for key in ('release','parent_release'):
            if records.get(key) is not None:
                r=self._record(ReleaseRecord,records[key]);logic.releases[r.release_id]=r
        ...
        logic.target_final_proposal_id=u256(view.get('proposal_id',0));logic.target_final_candidate_hash=str(view.get('candidate_hash',''));logic.target_final_release_id=str(view.get('release_id',''));logic.target_final_mode=str(view.get('mode',''));logic.target_final_kernel_hash=str(view.get('kernel_hash',''))
        nonfinal=data.get('target_nonfinal',{})
        logic.target_nonfinal_proposal_id=u256(nonfinal.get('proposal_id',0));logic.target_nonfinal_candidate_hash=str(nonfinal.get('candidate_hash',''))

    def _patch(self, logic: ProofPatchInstallLifecycleLogic, operation: str) -> dict[object, object]:
        ...
        for target,value in logic.active_proposal_by_target.items(): out['active'].append([str(target),int(value)])
        return out
    ...
    def execute(self, operation: str, request: str) -> None:
        ...
        if operation not in ('cancel','expire','confirm_install','reconcile_install'): raise gl.vm.UserError('Unsupported install lifecycle operation')
        data=json.loads(request);logic=ProofPatchInstallLifecycleLogic();self._load(logic,data);args=data.get('args',[])
        if operation=='cancel': logic.cancel(*args)
        elif operation=='expire': logic.expire(*args)
        elif operation=='confirm_install': logic.confirm_install(*args)
        else: logic.reconcile_install(*args)
