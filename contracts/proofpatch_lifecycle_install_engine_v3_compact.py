...
class ProofPatchInstallLifecycleLogic:
	...
	def reconcile_install(self,proposal_id:u256)->_A:
		proposal=self._require_proposal(proposal_id);self._require_policy_owner(proposal.target)
		if proposal.status!=STATUS_QUEUED:raise gl.vm.UserError('Proposal is not awaiting installation')
...
class ProofPatchInstallLifecycleEngine(gl.Contract):
	...
	def _load(self,logic:ProofPatchInstallLifecycleLogic,data:dict[object,object])->_A:
		E='candidate_hash';D='proposal_id';C='active_target';B='proposal';A='policy';logic.actor=Address(data['actor']);logic.now=int(data['now']);records=data.get('records',{});logic.proposal_count=u256(data.get(_C,0));logic.release_count=u256(data.get(_D,0));logic.installed_candidate_hashes=data.get(_E,{})
	...
	def execute(self,operation:str,request:str)->_A:
		C='confirm_install';B='expire';A='cancel'
		...
		if operation not in(A,B,C,'reconcile_install'):raise gl.vm.UserError('Unsupported install lifecycle operation')
